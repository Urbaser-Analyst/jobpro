...
const API_URL = 'https://script.google.com/macros/s/AKfycbzoCTndeeDCSkDbiG6NPo4TJIkHe_pxmIqsumFc9Rn8JzYLzq2UC-58k7Es2zpF1efTxA/exec';
...
async function apiCall(action, payload, _attempt) {
  ...
  console.log(
  `[API] ${action}: ${(performance.now() - started).toFixed(0)} ms`
);
return json.data;
  return json.data;
}
 ...
 */
/**
 * Our stored Drive image URLs look like:
 *   https://drive.google.com/thumbnail?id=FILE_ID&sz=w800
 * Google generates the thumbnail server-side at whatever size is
 * requested, so small UI elements (grid cards, table thumbnails) should
 * ask for a small size instead of downloading the same big version used
 * in detail views — this is what actually makes image-heavy pages like
 * the MRF product browser feel fast.
 */
