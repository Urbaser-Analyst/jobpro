  ...
  async render(mount) {
    ...
    mount.innerHTML = `
      ...
      <div class="card">
        ...
        <div class="flex-between" style="margin-top:12px;" id="productPager"></div>
      </div>
    `;
    ...
    if (canAdd) {
      ...
      imagePickers = [
        ImagePicker.mount(mount.querySelector('#img1'), { label: 'Image 1', slot: 1, itemCodeGetter: codeGetter }),
        ImagePicker.mount(mount.querySelector('#img2'), { label: 'Image 2', slot: 2, itemCodeGetter: codeGetter }),
        ImagePicker.mount(mount.querySelector('#img3'), { label: 'Image 3', slot: 3, itemCodeGetter: codeGetter }),
      ];
    }
    ...
    function applyAndRender() {
      ...
      const pageItems = list.slice((productPage - 1) * PRODUCT_PAGE_SIZE, productPage * PRODUCT_PAGE_SIZE);

      mount.querySelector('#countLabel').textContent = `(${list.length} of ${allProducts.length})`;
      tbody.innerHTML = rowsHtml(pageItems);
      bindRateInputs();
      ...
      const pager = mount.querySelector('#productPager');
      pager.innerHTML = `
        <span class="muted" style="font-size:12.5px;">Page ${productPage} of ${totalPages}</span>
        <div class="flex gap-8">
          <button type="button" class="btn btn-outline btn-sm" id="productPrevPage" ${productPage <= 1 ? 'disabled' : ''}>Previous</button>
          <button type="button" class="btn btn-outline btn-sm" id="productNextPage" ${productPage >= totalPages ? 'disabled' : ''}>Next</button>
        </div>`;
      const prevBtn = pager.querySelector('#productPrevPage');
      const nextBtn = pager.querySelector('#productNextPage');
      if (prevBtn) prevBtn.addEventListener('click', () => { productPage--; applyAndRender(); });
      if (nextBtn) nextBtn.addEventListener('click', () => { productPage++; applyAndRender(); });
    }
    ...
    mount.querySelector('#searchBox').addEventListener('input', (e) => { searchQ = e.target.value; productPage = 1; applyAndRender(); });
    mount.querySelector('#brandFilter').addEventListener('change', (e) => { filterBrand = e.target.value; productPage = 1; applyAndRender(); });
    mount.querySelector('#categoryFilter').addEventListener('change', (e) => { filterCategory = e.target.value; productPage = 1; applyAndRender(); });
    mount.querySelector('#clearFilters').addEventListener('click', () => {
      searchQ = ''; filterBrand = ''; filterCategory = ''; productPage = 1;
    ...
    if (form) {
      form.addEventListener('submit', async (e) => {
        ...
        try {
          // Images only ever touch Drive here — after Add product is
          // clicked, one slot at a time, in order. Nothing uploads on
          // mere selection, and there's no way to save the product
          // before an in-progress upload finishes since we await each
          // one before calling productMaster.add.
          values.image = '';
          values.image2 = '';
          values.image3 = '';

          if (imagePickers[0] && imagePickers[0].hasPendingFile()) {
            btn.textContent = 'Uploading image 1…';
            values.image = await imagePickers[0].upload();
          }
          if (imagePickers[1] && imagePickers[1].hasPendingFile()) {
            btn.textContent = 'Uploading image 2…';
            values.image2 = await imagePickers[1].upload();
          }
          if (imagePickers[2] && imagePickers[2].hasPendingFile()) {
            btn.textContent = 'Uploading image 3…';
            values.image3 = await imagePickers[2].upload();
          }
