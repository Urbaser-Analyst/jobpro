 ...
 *   Store to issue against).
 * - Store Admin / Admin / MRF Approver & Viewer see a Pending Approvals
 *   queue with Approve/Reject.
 *
 * PERFORMANCE NOTES (nothing below changes any business rule, permission,
 * or workflow — only how/when data is fetched and rendered):
 *  - The initial page load no longer fetches the full Product Master.
 *    It's only needed for the "Create MRF" combo/browser, so it's
 *    fetched in the background after the page is already interactive,
 *    and the combo/browse controls just show "Loading products…" until
 *    it arrives.
 *  - mrf.list now returns lightweight summaries (header fields + a line
 *    COUNT) instead of every line's full detail for every MRF. Full line
 *    detail is fetched on demand via mrf.get, only for the one MRF
 *    someone actually opens.
 *  - The Pending Approval queue renders immediately from the summary
 *    list, then fills in each card's line items progressively as
 *    mrf.get resolves for it, so approvers see the queue instantly
 *    instead of waiting on every pending MRF's detail before anything
 *    shows.
 *  - The main MRF table is paginated client-side (20 rows/page) so
 *    rendering cost doesn't grow with total MRF count.
 * -----------------------------------------------------------------------
 */
...
Dashboard.modules.mrf = {
  ...
  async render(mount) {
    ...
    // Fast initial batch — NOT including Product Master.
    const [config, fullList, pending] = await Promise.all([
      apiCall('config.get'),
    ...
    let products = null; // lazy-loaded, see ensureProducts()
    ...
    mount.innerHTML = `
      <div class="card" id="createMrfCard">
        ...
        <div class="form-row" style="align-items:flex-end;">
          ...
          <div class="field" style="align-self:flex-end;">
            <button type="button" class="btn btn-outline" id="addLineBtn" style="width:100%;" disabled>+ Quick add</button>
          </div>
        </div>
        <button type="button" class="btn btn-navy" id="browseBtn" style="margin-top:8px;" disabled>🛒 Browse products</button>
        <span class="muted" id="productsLoadingHint" style="font-size:11.5px; margin-left:8px;">Loading products…</span>
      ...
      <div class="card">
        ...
        <div class="flex-between" style="margin-top:12px;" id="mrfPager"></div>
      </div>
    `;

    // ===================== LAZY PRODUCT MASTER LOAD =====================
    function ensureProducts() {
      if (products) return Promise.resolve(products);
      if (!productsLoadingPromise) {
        productsLoadingPromise = apiCall('productMaster.list').then(list => {
          products = list;
          combo.setProducts(products);
          mount.querySelector('#addLineBtn').disabled = false;
          mount.querySelector('#browseBtn').disabled = false;
          mount.querySelector('#productsLoadingHint').style.display = 'none';
          return products;
        });
      }
    ...
    const combo = ProductCombo.mount(mount.querySelector('#mrfCombo'), [], (product) => {
      pendingProduct = product;
    });
    ensureProducts(); // fire in the background — page is already usable without it

    function addLineToDraft(product, qty) {
      const existing = draftLines.find(l => l.itemCode === product['Item Code']);
      if (existing) existing.qty += qty;
      else draftLines.push({
        itemCode: product['Item Code'], itemName: product['Item Name'],
        uom: product['UOM'], rate: Number(product['Rate'] || 0), qty: qty
      });
      renderDraft();
    }

    mount.querySelector('#browseBtn').addEventListener('click', async () => {
      const list = await ensureProducts();
      ProductBrowser.open(list, (product, qty) => addLineToDraft(product, qty), draftLines);
    });
    ...
    function renderDraft() {
      ...
      tbody.querySelectorAll('.remove-line').forEach(btn => {
        btn.addEventListener('click', () => { draftLines.splice(Number(btn.dataset.i), 1); renderDraft(); });
      });
    }
    ...
    function statusChip(status) {
      const map = {
        ...
        OPEN: ['chip-open', 'Open'],
        FULLY_ISSUED: ['chip-done', 'Fully Issued'],
    ...
    // ===================== PENDING APPROVALS (progressive fill) =====================
    if (isApprover) {
      const list = mount.querySelector('#pendingApprovalList');
      if (!pending.length) {
        list.innerHTML = `<div class="empty-state">Nothing waiting on approval.</div>`;
      } else {
        // Render instantly from the lightweight summary; line items fill
        // in as each mrf.get resolves, so the queue never waits on the
        // slowest one before showing anything.
        list.innerHTML = pending.map(m => `
          <div class="card" style="box-shadow:none; border:1px solid var(--line); margin-bottom:10px;" id="pend-${escapeHtml(m['MRF Id'])}">
            ...
            <div class="muted" style="font-size:12.5px; margin:10px 0;" data-role="items">Loading items…</div>
            <div class="flex gap-8">
              <button type="button" class="btn btn-primary btn-sm approve-btn" data-id="${escapeHtml(m['MRF Id'])}" disabled>Approve</button>
              <button type="button" class="btn btn-danger btn-sm reject-btn" data-id="${escapeHtml(m['MRF Id'])}" disabled>Reject</button>
            </div>
          </div>
        `).join('');

        pending.forEach(m => {
          apiCall('mrf.get', { mrfId: m['MRF Id'] }).then(detail => {
            const card = mount.querySelector(`#pend-${CSS.escape(m['MRF Id'])}`);
            if (!card) return; // page navigated away before this resolved
            card.querySelector('[data-role="items"]').outerHTML = `
              <ul style="margin:10px 0; padding-left:18px; font-size:13px;">
                ${detail.lines.map(l => `<li>${escapeHtml(l.itemCode)} — ${escapeHtml(l.itemName)} × ${l.qtyRequested} ${escapeHtml(l.uom)}</li>`).join('')}
              </ul>`;
            card.querySelector('.approve-btn').disabled = false;
            card.querySelector('.reject-btn').disabled = false;
          }).catch(err => {
            const card = mount.querySelector(`#pend-${CSS.escape(m['MRF Id'])}`);
            if (card) card.querySelector('[data-role="items"]').textContent = 'Could not load items: ' + err.message;
          });
        });
    ...
    // ===================== MAIN MRF LIST (sort/filter/search + pagination) =====================
    let mrfSortKey = 'Created At', mrfSortDir = -1, mrfSearchQ = '', mrfStatusF = '', mrfPage = 1;
    ...
    function renderMrfList() {
      ...
      const totalPages = Math.max(1, Math.ceil(list.length / MRF_PAGE_SIZE));
      if (mrfPage > totalPages) mrfPage = totalPages;
      const pageItems = list.slice((mrfPage - 1) * MRF_PAGE_SIZE, mrfPage * MRF_PAGE_SIZE);
      ...
      if (!pageItems.length) {
        ...
        rowsBody.innerHTML = pageItems.map(m => `
        ...
        rowsBody.querySelectorAll('.mrf-row').forEach(row => {
          row.addEventListener('click', () => openMrfDetail(row.dataset.id));
        });
      }
      ...
      const pager = mount.querySelector('#mrfPager');
      pager.innerHTML = `
        <span class="muted" style="font-size:12.5px;">Page ${mrfPage} of ${totalPages}</span>
        <div class="flex gap-8">
          <button type="button" class="btn btn-outline btn-sm" id="mrfPrevPage" ${mrfPage <= 1 ? 'disabled' : ''}>Previous</button>
          <button type="button" class="btn btn-outline btn-sm" id="mrfNextPage" ${mrfPage >= totalPages ? 'disabled' : ''}>Next</button>
        </div>`;
      const prevBtn = pager.querySelector('#mrfPrevPage');
      const nextBtn = pager.querySelector('#mrfNextPage');
      if (prevBtn) prevBtn.addEventListener('click', () => { mrfPage--; renderMrfList(); });
      if (nextBtn) nextBtn.addEventListener('click', () => { mrfPage++; renderMrfList(); });
    }
    ...
    mount.querySelector('#mrfSearch').addEventListener('input', e => { mrfSearchQ = e.target.value; mrfPage = 1; renderMrfList(); });
    mount.querySelector('#mrfStatusFilter').addEventListener('change', e => { mrfStatusF = e.target.value; mrfPage = 1; renderMrfList(); });

    renderMrfList();

    // ===================== LAZY DETAIL MODAL =====================
    async function openMrfDetail(mrfId) {
      Modal.open(`<div class="modal-header"><h2>Loading…</h2></div><p class="muted">Fetching MRF ${escapeHtml(mrfId)}…</p>`);
      try {
        const m = await apiCall('mrf.get', { mrfId });
        Modal.open(`
