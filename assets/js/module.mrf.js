...
Dashboard.modules.mrf = {
  ...
  async render(mount) {
    ...
    const [products, config, mine, pending] = await Promise.all([
      apiCall('productMaster.list'),
      apiCall('config.get'),
      apiCall('mrf.list', { scope: 'mine' }),
    ...
    mount.innerHTML = `
      <div class="card">
        ...
        <div class="form-row" style="align-items:flex-end;">
          ...
          <div class="field" style="align-self:flex-end;">
            <button type="button" class="btn btn-outline" id="addLineBtn" style="width:100%;">+ Add line</button>
          </div>
        </div>
      ...
      <div class="card">
        ...
        <div class="table-wrap">
          <table>
            <thead><tr><th>MRF ID</th><th>Status</th><th>Total Value</th><th>Lines</th><th>Created</th>${canSeeAll ? '<th>Requested by</th>' : ''}</tr></thead>
    ...
    mount.querySelector('#addLineBtn').addEventListener('click', () => {
      ...
      const existing = draftLines.find(l => l.itemCode === pendingProduct['Item Code']);
      if (existing) {
        existing.qty += qty;
      } else {
        draftLines.push({
          itemCode: pendingProduct['Item Code'],
          itemName: pendingProduct['Item Name'],
          uom: pendingProduct['UOM'],
          rate: Number(pendingProduct['Rate'] || 0),
          qty: qty
        });
      }
      renderDraft();
    ...
    mount.querySelector('#submitMrfBtn').addEventListener('click', async () => {
      ...
      try {
        ...
        btn.disabled = false; btn.textContent = 'Confirm & Submit MRF';
      }
    });
    ...
    // ---- My/All MRFs list ----
    const list = canSeeAll ? await apiCall('mrf.list', { scope: 'all' }) : mine;
    mount.querySelector('#mrfCountLabel').textContent = `(${list.length})`;
    ...
    if (!list.length) {
      rowsBody.innerHTML = `<tr><td colspan="${canSeeAll ? 6 : 5}"><div class="empty-state">No MRFs yet.</div></td></tr>`;
    } else {
      ...
      rowsBody.querySelectorAll('.mrf-row').forEach(row => {
        row.addEventListener('click', () => {
          const m = list.find(x => x['MRF Id'] === row.dataset.id);
