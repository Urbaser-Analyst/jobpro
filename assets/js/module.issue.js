 ...
 *
 * PERFORMANCE NOTES (same approach as module.mrf.js — no business rule,
 * permission, or workflow changes, only how/when data is fetched):
 *  - "Awaiting your verification" stays fully detailed on load — it's a
 *    small, personal, immediately-actionable queue, so there's nothing
 *    to gain by deferring it.
 *  - The main Issues list (mine/all) now fetches a lightweight summary
 *    (header fields + a line COUNT) instead of every line's full detail
 *    for every Issue. Full detail loads on demand via issue.get, only
 *    for the one Issue someone actually opens.
 *  - The "Defective returns to acknowledge" queue now comes from its own
 *    dedicated, cheap backend query instead of being derived by
 *    scanning every issue's full line list on the frontend.
 *  - The main Issues table is paginated client-side (20 rows/page).
 * -----------------------------------------------------------------------
 */
...
Dashboard.modules.issue = {
  ...
  async render(mount) {
    ...
    const [eligibleMRFs, users, toReceive, fullList, pendingDefects] = await Promise.all([
      ...
      canCreate ? apiCall('issue.pendingDefects') : Promise.resolve([])
    ]);
    ...
    mount.innerHTML = `
      ...
      <div class="card">
        ...
        <div class="flex-between" style="margin-top:12px;" id="issuePager"></div>
      </div>
    `;
    ...
    // ===================== AWAITING MY VERIFICATION (kept fully detailed — small, actionable) =====================
    if (toReceive.length) {
    ...
    // ===================== DEFECTIVE RETURNS TO ACKNOWLEDGE (own dedicated query) =====================
    if (canCreate) {
    ...
    // ===================== MAIN ISSUE LIST (sort/filter/search + pagination) =====================
    let sortKey = 'Created At', sortDir = -1, searchQ = '', statusF = '', issuePage = 1;
    const rowsBody = mount.querySelector('#issueListRows');

    function renderList() {
      let list = fullList.filter(i => {
      ...
      const totalPages = Math.max(1, Math.ceil(list.length / ISSUE_PAGE_SIZE));
      if (issuePage > totalPages) issuePage = totalPages;
      const pageItems = list.slice((issuePage - 1) * ISSUE_PAGE_SIZE, issuePage * ISSUE_PAGE_SIZE);

      mount.querySelector('#issueCountLabel').textContent = `(${list.length} of ${fullList.length})`;
      if (!pageItems.length) {
        rowsBody.innerHTML = `<tr><td colspan="6"><div class="empty-state">No issues match.</div></td></tr>`;
      } else {
        rowsBody.innerHTML = pageItems.map(i => `
      ...
      const pager = mount.querySelector('#issuePager');
      pager.innerHTML = `
        <span class="muted" style="font-size:12.5px;">Page ${issuePage} of ${totalPages}</span>
        <div class="flex gap-8">
          <button type="button" class="btn btn-outline btn-sm" id="issuePrevPage" ${issuePage <= 1 ? 'disabled' : ''}>Previous</button>
          <button type="button" class="btn btn-outline btn-sm" id="issueNextPage" ${issuePage >= totalPages ? 'disabled' : ''}>Next</button>
        </div>`;
      const prevBtn = pager.querySelector('#issuePrevPage');
      const nextBtn = pager.querySelector('#issueNextPage');
      if (prevBtn) prevBtn.addEventListener('click', () => { issuePage--; renderList(); });
      if (nextBtn) nextBtn.addEventListener('click', () => { issuePage++; renderList(); });
    }
    ...
    mount.querySelector('#issueSearch').addEventListener('input', e => { searchQ = e.target.value; issuePage = 1; renderList(); });
    mount.querySelector('#issueStatusFilter').addEventListener('change', e => { statusF = e.target.value; issuePage = 1; renderList(); });
    renderList();

    // ===================== LAZY DETAIL MODAL =====================
    async function openIssueDetail(issueId) {
      Modal.open(`<div class="modal-header"><h2>Loading…</h2></div><p class="muted">Fetching Issue ${escapeHtml(issueId)}…</p>`);
      try {
        const i = await apiCall('issue.get', { issueId });
        Modal.open(`
